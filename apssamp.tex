...
$k$ and $d$. 
The proposed methods are evaluated against an exact dynamic programming algorithm, two approximate approaches, namely beam search dynamic programming and tabu search, and the mathematical optimization solver SCIP. Exact
...
\begin{enumerate}
    ...
    $k$-neighbor tensor networks, including an analysis of their computational
...
This section presents a series of experiments designed to evaluate the
accuracy and computational performance of the proposed algorithms. For
consistency with the QUBO case, the linear coefficients $D_i$ are set to zero. The matrix-based method is implemented using
NumPy~\cite{numpy}, whereas the tensor-based method is implemented in
TensorKrowch~\cite{tensorkrowch}. 

The experimental evaluation is divided into two parts. First, the
accuracy and reliability of the proposed methods are assessed using
exact dynamic programming as the reference, and their performance is
compared with the heuristic dynamic-programming method. Second,
the computational scaling of the matrix-based, tensor-based, exact
dynamic-programming, and heuristic dynamic-programming methods are
studied as a function of the number of variables $n$, the interaction
range $k$, and the local dimension $d$.

All methods are evaluated on the same randomly generated $k$-neighbors
QUDO instances, whose interaction coefficients are independently drawn
from the continuous uniform distribution $\mathcal{U}(-10,10)$. For each
configuration, the accuracy experiment aggregates 50 independently
generated instances, whereas the scaling experiments aggregate three.
Each method is executed once per instance. The experiments were run on
an HP Victus 16-r0xxx laptop equipped with a 13th-generation Intel Core
i7-13700H processor (14 cores and 20 logical processors) and 32~GB of
RAM, under Ubuntu on WSL2. The accuracy table reports arithmetic means
over the 50 instances; the corresponding sample standard deviations are
computed in the processed results. The scaling plots report the median
execution time over the three instances. No within-instance timing
repetitions or variability bands are included.


\subsection{Accuracy and reliability}\label{ssec:accuracy-evaluation}

The first experiment evaluates the accuracy and reliability of the
matrix-based and tensor-based methods using exact dynamic programming as
the reference. Since exact dynamic programming guarantees the global
optimum for the $k$-neighbors QUDO problem, it provides a direct means
of measuring the quality of the approximate solutions. The proposed
methods are also compared with heuristic dynamic programming to assess
their relative trade-off between solution quality and execution time.

Let $C^*$ denote the optimal cost obtained by exact dynamic programming
and $C_{\mathrm{alg}}$ the cost returned by the algorithm under
evaluation. The relative error is defined as
\begin{equation}
    \epsilon_{\mathrm{rel}}
    =
...
\end{equation}
The small constant $\varepsilon_0$ prevents division by zero when the
optimal cost is zero or numerically close to zero. For
$\left|C^*\right|>\varepsilon_0$, this expression reduces to the
standard relative error and is invariant under a common rescaling of
the objective values. Thus, $\epsilon_{\mathrm{rel}}=0$ indicates that
the algorithm has recovered an optimal objective value, whereas larger
values indicate a greater deviation from the optimum. 

Table~\ref{tab:experimental-summary} summarizes the results of this
experiment for different values of the number of variables $n$, the
interaction range $k$, and the local dimension $d$. Each row aggregates
50 independently generated instances evaluated by all methods.
The table reports the relative error and execution time of the
matrix-based, tensor-based, and heuristic dynamic-programming methods.
The exact dynamic-programming solution is used only as the reference
optimum and therefore has zero relative error by definition.

These results make it possible to compare both the accuracy and
computational cost of the approximate methods. In particular, they show
how often each method approaches or recovers the exact solution and the
additional execution time required to obtain that level of accuracy.
This comparison is especially relevant for configurations in which
exact dynamic programming becomes computationally expensive, as it
allows the relative advantages of the tensor-network methods and the
dynamic-programming heuristic to be identified.

\begin{table*}[t]
    \centering
    \caption{
    Comparison against exact dynamic programming on random
    $k$-neighbor QUDO instances. Each row aggregates 50 independently
    generated instances. Lower relative error and execution time are
    better.
    }
...
The second part of the experimental evaluation studies the computational
scaling of the tensor, matrix, and exact dynamic-programming methods.
The execution time is analyzed as a function of the interaction range
$k$, the local dimension $d$, and the number of variables $n$. These
experiments complement the theoretical complexity analysis presented in
Sec.~\ref{sssec:kneigh complexity} and reveal the practical influence of
implementation-dependent constant factors.

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.9\textwidth]
        {Images/experiment_3_kd_t.png}
    \caption{Execution time of the tensor, matrix, and exact
    dynamic-programming methods as a function of (a) the interaction
    range $k$, with $n=100$ and $d=2$, and (b) the local dimension $d$,
    with $n=1000$ and $k=2$. In both experiments, $\tau=400$.}
    \label{fig:scaling-kd}
\end{figure*}

Figure~\ref{fig:scaling-kd} examines the dependence of the execution
time on $k$ and $d$ while the remaining problem parameters are fixed.
For small interaction ranges, exact dynamic programming and the matrix
formulation have lower execution times than the tensor formulation.
However, the cost of exact dynamic programming increases rapidly with
$k$, because the number of boundary states grows as $d^k$. The
matrix-based method also becomes progressively more expensive, whereas
the tensor formulation exhibits a slower increase over the evaluated
range. Consequently, the tensor method becomes competitive with the
other methods for sufficiently large interaction ranges.

A similar behavior is observed when increasing the local dimension
$d$. The execution time of all methods increases because each variable
can take a larger number of values. The matrix method remains the
fastest implementation over the evaluated range, reflecting the
efficiency of optimized matrix--vector operations. Nevertheless, the
execution times of exact dynamic programming and the tensor method grow
more rapidly, and the tensor formulation eventually becomes faster than
exact dynamic programming for the largest tested dimensions. These
results show that asymptotic complexity alone does not determine the
fastest implementation: the crossover points also depend on constant
factors, the data structures employed, and the ability of each method
to exploit sparsity.
...
\begin{figure*}[t]
    ...
    \caption{Execution time as a function of the number of variables
    $n$ for different local dimensions $d$, with fixed interaction
    range $k=2$. The panels show the matrix, tensor, and exact
    dynamic-programming methods, respectively.}
    \label{fig:scaling-n-d}
\end{figure*}

Figure~\ref{fig:scaling-n-d} studies the dependence on the number of
variables for $d\in\{2,\ldots,8\}$ and a fixed interaction range.
For every fixed value of $d$, the execution time grows approximately
linearly with $n$ for all three methods. This behavior is consistent
with their theoretical complexities, in which $n$ appears as a linear
factor when $k$ and $d$ are fixed. Increasing $d$ mainly increases the
slope of the curves, with a particularly visible effect for the tensor
and exact dynamic-programming methods. The matrix formulation has the
smallest execution time for every configuration considered in this
experiment, although all three implementations retain approximately
linear scaling with the number of variables.
...
\begin{figure*}[t]
    ...
    \caption{Execution time as a function of the number of variables
    $n$ for different interaction ranges $k$, with fixed local
    dimension $d=2$. The panels show the matrix, tensor, and exact
    dynamic-programming methods, respectively.}
    \label{fig:scaling-n-k}
\end{figure*}

The influence of the interaction range is examined further in
Fig.~\ref{fig:scaling-n-k}, where $n$ is varied for
$k\in\{2,\ldots,10\}$ at fixed local dimension. Once again, the
execution time is approximately linear in $n$ for each fixed value of
$k$. However, the slope increases with the interaction range because
more interactions must be incorporated into each contraction or
dynamic-programming transition. This increase is especially pronounced
for exact dynamic programming, whose state space grows as $d^k$. At the
largest tested interaction range, the tensor formulation becomes faster
than exact dynamic programming, whereas the matrix implementation
retains the lowest execution time throughout the evaluated
configurations.

Overall, the experiments confirm two distinct scaling behaviors. For
fixed $k$ and $d$, all methods scale approximately linearly with the
number of variables. In contrast, increasing either the interaction
range or the local dimension produces a substantially stronger increase
in execution time. Although the matrix formulation is the fastest
implementation for most of the tested configurations, the tensor method
becomes increasingly competitive as $k$ grows and can outperform exact
dynamic programming for the largest interaction ranges. This behavior
is consistent with the theoretical analysis and motivates the use of
tensor-network formulations for problems with larger local interaction
structures.

\begin{table*}[t]
    \centering
    \caption{
    Comparison of TN, Beam DP, Tabu search, and SCIP for random
    $k$-neighbor QUDO instances with $n=500$. Errors are mean relative
    errors, with the best result in each row highlighted in bold.
    Runtime ratios are computed with respect to TN; values below one
    indicate a faster method.
    }
...
\begin{table}[t]
    \centering
    \caption{Runtime comparison between the matrix-based method and SCIP. 
    The success rate indicates the fraction of instances for which SCIP reached 
    the reference solution. Runtime and speedup values are medians computed over 
    the successful instances.}
    \label{tab:matrix_scip_comparison}
    \begin{tabular}{rrrrrrrr}
    \toprule
    $n$ & $d$ & $k$ &
    Reached & Success &
    Matrix (s) & SCIP (s) & Speedup \\
    \midrule
